...
export async function POST(request: Request) {
  try {
    ...
    return createInquiryHandler({ repository: createInquiryRepository(database), secret: env.SUPABASE_SERVICE_ROLE_KEY, vercel: process.env.VERCEL === "1", localDevelopment: process.env.NODE_ENV === "development" })(request);
