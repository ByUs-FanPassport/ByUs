...
export function BlockchainJobsManager() {
  ...
  return <AdminOperationsShell locale={locale}>
    ...
    <form className={styles.filterBar} action={applyFilters}>
      <label><span>{locale === "ko" ? "상태" : "Status"}</span><select name="status" defaultValue={status ?? ""}><option value="">{t.all}</option>{statusOptions.map((item) => <option key={item}>{item}</option>)}</select></label>
      <label className={styles.growField}><span>{t.search}</span><input name="jobId" defaultValue={jobId} placeholder="00000000-0000-0000-0000-000000000000" /></label>
    ...
    {selected && <><div className={styles.drawerBackdrop} onClick={() => setSelected(null)} /><aside className={styles.drawer} role="dialog" aria-modal="true" aria-labelledby="job-detail-title"><div className={styles.drawerHeader}><div><p>{selected.id}</p><h2 id="job-detail-title">{t.detail}</h2></div><button className={styles.iconButton} type="button" aria-label={t.close} onClick={() => setSelected(null)}><X aria-hidden="true" /></button></div><div className={styles.drawerBody}>
    ...
    </div><footer className={styles.drawerFooter}>{session.admin.role === "viewer" && <p>{t.viewer}</p>}{!canRetry && session.admin.role !== "viewer" && <p>{t.retryUnavailable}</p>}<button className={styles.primaryButton} type="button" disabled={!canRetry} onClick={() => setConfirming(true)}><RefreshCw aria-hidden="true" />{t.retry}</button></footer></aside></>}
    {confirming && <div className={styles.confirmBackdrop} role="presentation"><section className={styles.confirmDialog} role="alertdialog" aria-modal="true" aria-labelledby="retry-title" aria-describedby="retry-description"><h2 id="retry-title">{t.confirmTitle}</h2><p id="retry-description">{t.confirmBody}</p>{retryError && <p className={styles.inlineError} role="alert">{t.retryError}</p>}<div><button className={styles.secondaryButton} type="button" disabled={retrying} onClick={() => setConfirming(false)}>{t.cancel}</button><button className={styles.dangerButton} type="button" disabled={retrying} onClick={() => void retry()}>{retrying ? "…" : t.confirm}</button></div></section></div>}
