...
vi.mock("next/navigation", () => ({
  usePathname: () => "/admin/notice-comments",
...
import { NoticeCommentManager } from "./notice-comment-manager";
...
describe("NoticeCommentManager pagination", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    roleState.current = "viewer";
  });
  ...
  it("moves through opaque cursors and returns through the cursor stack", async () => {
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
      ...
      return {
        ...
        json: async () => url.includes("cursor=cursor-1")
          ? { comments: [comment("22222222-2222-4222-8222-222222222222", "두 번째 페이지")], nextCursor: null }
          : { comments: [comment("11111111-1111-4111-8111-111111111111", "첫 번째 페이지")], nextCursor: "cursor-1" },
      } as Response;
    }));

    render(<NoticeCommentManager />);
    ...
    expect(vi.mocked(fetch)).toHaveBeenLastCalledWith(
      "/api/admin/notice-comments?cursor=cursor-1",
  ...
  it("keeps the previous-page escape available after a page error", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
      ...
      return {
        ok: true,
        status: 200,
        json: async () => ({ comments: [comment("11111111-1111-4111-8111-111111111111", "첫 페이지")], nextCursor: "cursor-1" }),
      } as Response;
    }));
  ...
  it("blocks page changes while a hide mutation is in flight", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return {
        ok: true,
        status: 200,
        json: async () => ({ comments: [comment("11111111-1111-4111-8111-111111111111", "검토할 댓글")], nextCursor: "cursor-1" }),
      } as Response;
    }));

    render(<NoticeCommentManager />);
    await screen.findByText("검토할 댓글");
    ...
    fireEvent.click(screen.getByRole("button", { name: "댓글 숨기기" }));
