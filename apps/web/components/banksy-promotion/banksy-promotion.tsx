...
function CampaignCopy({ raffle, locale, modal = false }: { raffle: Raffle; locale: "ko" | "en"; modal?: boolean }) {
  const ko = locale === "ko";
  return <>
    <span className={styles.eyebrow}><span className={styles.status}>{ko ? "응모 진행 중" : "GIVEAWAY OPEN"}</span><span>ELINA × BANKSY</span></span>
    <h2 id={modal ? "banksy-promotion-title" : undefined}>{ko ? <>뱅크시를 만나는{" "}<br />특별한 기회.</> : <>Your chance{" "}<br />to see Banksy.</>}</h2>
    <p className={styles.description}>{ko ? <>더현대 서울 뱅크시 전시 관람권<br /><strong>{raffle.winnerQuantity}명 추첨</strong> · 엘리나와 함께하는 이벤트</> : <>Banksy exhibition at The Hyundai Seoul<br /><strong>{raffle.winnerQuantity} winners</strong> · A giveaway with Elina</>}</p>
    {raffle.entryClosesAt && <p className={styles.deadline}><time dateTime={raffle.entryClosesAt}>{formatRaffleDateTime(raffle.entryClosesAt, locale)}</time> {ko ? "마감" : "closes"}</p>}
  </>;
}

export function BanksyFanBanner({ available, locale }: { available: readonly Raffle[]; locale: "ko" | "en" }) {
  const raffle = available.find(item => item.benefitId === banksyTicketId);
  if (!raffle) return null;
  return <section className={styles.banner} aria-label={locale === "ko" ? "엘리나 뱅크시 이벤트" : "Elina Banksy giveaway"}>
    ...
    <Link className={styles.bannerAction} href={creatorRafflesHref("elina", locale)} onClick={markSeen}>
      <Ticket aria-hidden="true" /><span>{locale === "ko" ? "이벤트 응모하러 가기" : "Explore the giveaway"}</span><ArrowUpRight aria-hidden="true" />
    </Link>
  </section>;
}

function BanksyEntryPrompt({ locale }: { locale: "ko" | "en" }) {
  ...
  return <Dialog open={open} onClose={close} labelledBy="banksy-promotion-title" initialFocusRef={headingRef} backdropClassName={styles.backdrop} contentClassName={styles.dialog}>
    <button type="button" className={styles.close} onClick={close} aria-label={ko ? "이벤트 안내 닫기" : "Close giveaway"}><X aria-hidden="true" /></button>
    ...
    <div className={styles.dialogBody}>
      ...
      <Link className={styles.primaryAction} href={creatorRafflesHref("elina", locale)} onClick={close}>{ko ? "이벤트 응모하러 가기" : "Explore the giveaway"}<ArrowUpRight aria-hidden="true" /></Link>
      <p className={styles.entryNote}>{ko ? "선물을 고르고 엘리나 응모권으로 응모하세요." : "Choose a prize and enter with your Elina raffle tickets."}</p>
      <button className={styles.later} type="button" onClick={close}>{ko ? "지금은 둘러볼게요" : "Keep browsing"}</button>
    </div>
  </Dialog>;
}
