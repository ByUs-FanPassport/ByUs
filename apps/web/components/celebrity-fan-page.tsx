      ...
      <div className={styles.hubLayout}>
        <div className={styles.mainColumn}>
          ...
          <section className={styles.contentSection} id="live-activity" aria-labelledby="live-activity-title">
            ...
            {upcomingLive ? <div className={styles.liveSection}><div className={styles.liveCopy}><p><Radio aria-hidden="true" /> {upcomingLive.effectiveStatus === "live" ? (locale === "ko" ? "LIVE 진행 중" : "LIVE NOW") : (locale === "ko" ? "LIVE 예정" : "UPCOMING LIVE")}</p><h3>{upcomingLive.title}</h3><span><Clock aria-hidden="true" /> {formatLiveDate(upcomingLive.startsAt, locale)}</span></div><Link className={styles.liveAction} href={`/live/${upcomingLive.slug}${localeQuery}`} aria-label={`${t.liveDetails}: ${upcomingLive.title}`}><span><Play aria-hidden="true" /> {t.liveDetails}</span><ArrowRight aria-hidden="true" /></Link></div> : <div className={styles.inlineEmpty} role="status"><strong>{t.noLive}</strong><span>{t.noLiveHelp}</span></div>}
          </section>
