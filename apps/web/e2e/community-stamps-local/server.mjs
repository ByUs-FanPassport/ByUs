...
export async function startHarness(port=4193) {
 const server=await createServer({configFile:false,root:here,publicDir:path.join(web,'public'),cacheDir:path.join(web,'node_modules/.vite-community-stamps'),esbuild:{jsx:'automatic'},resolve:{alias:[{find:'@',replacement:web},{find:'@privy-io/react-auth',replacement:path.join(here,'privy.ts')},...['link','image'].map(name=>({find:`next/${name}`,replacement:path.join(here,`../mission-local/next-${name}.tsx`)}))]},server:{host:'127.0.0.1',port,strictPort:true,fs:{allow:[web,path.resolve(web,'../../node_modules')]}}});
