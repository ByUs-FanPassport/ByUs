...
export async function startHarness() {
  ...
  const dependencies = {
    ...
    rpc(name, args) {
      if (!["read_celebrity_lounge", "post_celebrity_lounge_message", "remove_owned_lounge_message", "set_lounge_message_reaction", "read_admin_lounge_messages", "hide_admin_lounge_message", "read_celebrity_notice_comments", "post_celebrity_notice_comment", "remove_owned_notice_comment"].includes(name)) throw new Error("Unknown Lounge RPC");
  ...
  handleApi = async (req, res, next) => {
    ...
    try {
      ...
      if (url.pathname === "/api/auth/session") {
        ...
        if (room) response = request.method === "GET" ? await handlers.read(request, room[1]) : await handlers.post(request, room[1]);
        ...
        else if (url.pathname.endsWith("/fan-activity-visibility")) response = fanpageJson({ enabled: false });
  ...
  await vite.listen();
  return { vite, baseURL: "http://127.0.0.1:4184", query, actors, dependencies };
}
