"use client";
import { messages } from "@/i18n/catalogs/features__analytics__client__banksy-campaign";
...
export function BanksyOutboundLinks({ locale, surface }: { locale: AppLocale; surface: "raffle_list" | "raffle_receipt" }) {
  ...
  return <nav className={styles.links} aria-label={locale === "ko" ? "관련 전시와 굿즈" : translate(locale, messages.banksy_related, "Exhibition and goods")}>
    <a href={campaignOutboundHref("exhibition", surface, visitId)} rel="nofollow noreferrer" onClick={event => prepare(event, "exhibition")} onAuxClick={event => prepare(event, "exhibition")}>{locale === "ko" ? "전시 자세히 보기" : translate(locale, messages.banksy_details, "Exhibition details")}<span aria-hidden="true"> ↗</span></a>
    <a href={campaignOutboundHref("goods", surface, visitId)} rel="nofollow noreferrer" onClick={event => prepare(event, "goods")} onAuxClick={event => prepare(event, "goods")}>{locale === "ko" ? "굿즈 방문 예약" : translate(locale, messages.banksy_shop, "Book a goods shop visit")}<span aria-hidden="true"> ↗</span></a>
  </nav>;
}
