...
function OwnerCollection({ locale, creator, resource }: { locale: Locale; creator?: string; resource: Resource }) {
  ...
  return <section className={styles.section} id="community-stamps" aria-labelledby="community-stamps-title">
    ...
    {resource.state.status === "loading" ? <p className={styles.status} role="status">{locale === "ko" ? "스탬프를 불러오고 있어요." : translate(locale, localizedMessages.m14ba0b095bf9, "Loading your Stamps.")}</p> : resource.state.status === "error" ? <p className={styles.status} role="alert">{locale === "ko" ? "스탬프를 불러오지 못했어요." : translate(locale, localizedMessages.m6f4e687b2a1d, "Couldn’t load your Stamps.")} <button className={styles.action} onClick={resource.retry}><RotateCcw aria-hidden="true"/>{locale === "ko" ? "다시 시도" : translate(locale, localizedMessages.mf721ef255caa, "Try again")}</button></p> : <>
      ...
      <ul className={styles.grid}>{kinds.map(kind => {
        ...
        return <li className={styles.card} data-earned={!!stamp} key={kind}>
          ...
          {stamp ? <button className={styles.action} onClick={() => setSelected(stamp)}>{locale === "ko" ? "스탬프 보기" : translate(locale, localizedMessages.mc4481ae2313e, "View Stamp")}</button> : kind === "welcome" ? <button className={styles.action} disabled={busy} onClick={() => void run("welcome")}>{locale === "ko" ? "가입 스탬프 받기" : translate(locale, localizedMessages.m41ade7763a1c, "Get welcome Stamp")}</button> : kind === "invite" ? <button className={styles.action} disabled={busy} onClick={() => { setInviteOpen(true); void run("invite-code"); }}>{locale === "ko" ? "친구 초대하기" : translate(locale, localizedMessages.ma5023c1cc581, "Invite a friend")}</button> : kind === "share" && creator ? <SharePassport creator={{ slug: creator, name: creator }} locale={locale}/> : <Link className={styles.action} href={href as Route}>{kind === "daily_checkin" ? (locale === "ko" ? "출석하러 가기" : translate(locale, localizedMessages.md29cb19a1b1d, "Check in")) : kind === "share" ? (locale === "ko" ? "패스포트 고르기" : translate(locale, localizedMessages.m3442db55dbf6, "Choose a Passport")) : (locale === "ko" ? "댓글 남기기" : translate(locale, localizedMessages.m2e60da1bfeff, "Leave a comment"))}</Link>}
        </li>;
      })}</ul>
