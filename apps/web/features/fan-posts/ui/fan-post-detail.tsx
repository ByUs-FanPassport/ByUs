...
function DetailForOwner({ postId, locale }: { postId: string; locale: AppLocale }) {
  const copy = contentCopy(locale), auth = usePrivy(), mutation = useContentMutation(locale), fieldId = useId();
  const [cursor, setCursor] = useState<string | null>(null), [body, setBody] = useState(""), [parentId, setParentId] = useState<string | null>(null);
  ...
  async function submit() {
    ...
    if (await mutation.request(`/api/posts/${postId}/comments`, "POST", { ...value, idempotencyKey: attempt.current.key })) {
      setBody(""); setParentId(null); setCursor(null); attempt.current = null; refresh();
    }
  }
  ...
  return <section className={styles.section}>
    <Link href={`${creatorHomeHref(data.celebritySlug)}?tab=community&locale=${locale}`} className={styles.link}>{copy.back}</Link>
    <h1>{copy.community}</h1><PostCard post={data} locale={locale} onChanged={refresh} detail />
    <section className={styles.section} aria-labelledby={`${fieldId}-heading`}><h2 id={`${fieldId}-heading`}>{copy.comments}</h2>
      {auth.ready && (auth.authenticated ? <form className={styles.composer} onSubmit={event => { event.preventDefault(); void submit(); }}>
        {parentId && <div className={styles.row}><span className={styles.status}>{copy.replying}</span><button type="button" className={styles.button} onClick={() => setParentId(null)}>{copy.cancel}</button></div>}
        <label htmlFor={fieldId}>{copy.newComment}<textarea id={fieldId} rows={3} required maxLength={1000} disabled={mutation.busy} value={body} onChange={event => setBody(event.target.value)} /></label>
      ...
      {comments.state.status === "ready" ? <>
        {!comments.state.data.items.length ? <p className={styles.empty}>{copy.newComment}</p> : <ul className={styles.list}>{comments.state.data.items.map(comment => <li key={comment.id} id={`comment-${comment.id}`} className={`${styles.card} ${comment.parentId ? styles.reply : ""}`}>
          ...
          <div className={styles.actions}>{auth.authenticated && !comment.parentId && <button type="button" disabled={mutation.busy} onClick={() => { setParentId(comment.id); document.getElementById(fieldId)?.focus(); }}>{copy.reply}</button>}{comment.isOwner && <button type="button" disabled={mutation.busy} onClick={() => void remove(comment.id)}>{copy.delete}</button>}</div>
          <ContentActions targetType="fan_post_comment" targetId={comment.id} locale={locale} canBlock={!comment.isOwner} onChanged={refresh} />
        </li>)}</ul>}
