...
function FeedForOwner({ slug, locale }: { slug: string; locale: AppLocale }) {
  const auth = usePrivy(), copy = contentCopy(locale), heading = useId(), [cursor, setCursor] = useState<string | null>(null);
  ...
  const refresh = () => { setCursor(null); resource.retry(); };
  ...
  return <section className={styles.section} aria-labelledby={heading}><h2 id={heading}>{copy.community}</h2>
    ...
    {resource.state.status === "ready" ? <>
      ...
      {(cursor || resource.state.data.nextCursor) && <nav className={styles.pagination} aria-label={copy.community}>{cursor && <button type="button" onClick={() => setCursor(null)}>{copy.newest}</button>}{resource.state.data.nextCursor && <button type="button" onClick={() => setCursor(resource.state.status === "ready" ? resource.state.data.nextCursor : null)}>{copy.older}</button>}</nav>}
