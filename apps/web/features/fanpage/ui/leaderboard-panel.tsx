...
export function LeaderboardPanel({ slug, locale }: { slug: string; locale: "ko" | "en" }) {
  ...
  if (state.status === "error" && state.code === "LEADERBOARD_NOT_AVAILABLE") return <><FanGatheringPanel slug={slug} locale={locale} /><p className={gatheringStyles.rankStatus}>{ko ? "패스포트 팬 501명부터 팬점수 순위도 볼 수 있어요." : "Fan Score rankings open at 501 Passport holders."}<span>{ko ? `현재 ${state.membershipCount ?? 0}명 / 501명` : `${state.membershipCount ?? 0} / 501 fans`}</span></p></>;
  ...
  return <><FanGatheringPanel slug={slug} locale={locale} /><section className={styles.leaderboard}><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>FAN LEADERBOARD</p><h2><Trophy aria-hidden="true" />{ko ? "함께 쌓은 팬 활동의 순위" : "Your fan activity, ranked"}</h2></div><span>TOP 100</span></div><p className={styles.intro}>{ko ? "누적 팬점수 기준 · 동점일 때는 패스포트를 먼저 만든 팬이 앞서요." : "Lifetime Fan Score · Earlier Passport issuance breaks ties."}</p>
