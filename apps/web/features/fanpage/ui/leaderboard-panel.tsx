...
export function LeaderboardPanel({ slug, locale }: { slug: string; locale: "ko" | "en" }) {
  ...
  if (state.status === "error" && state.code === "LEADERBOARD_NOT_AVAILABLE") return <section className={styles.locked}><LockKeyhole aria-hidden="true" /><p className={styles.eyebrow}>{ko ? "리더보드 · 집계 중" : "Leaderboard · Counting fans"}</p><h2>{ko ? "팬들이 모이면 순위가 열려요." : "The leaderboard opens as fans join."}</h2><p>{ko ? "ByUs 패스포트 보유 팬이 501명이 되면 누적 팬점수 Top 100과 내 순위를 확인할 수 있어요." : "Once 501 fans have a ByUs Passport, you can view the top 100 by lifetime Fan Score and see your rank."}</p><strong>{ko ? `현재 ${state.membershipCount ?? 0}명 / 501명` : `${state.membershipCount ?? 0} / 501 fans`}</strong></section>;
