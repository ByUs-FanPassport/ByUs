...
const auth = vi.hoisted(() => ({ ready: true, authenticated: true, user: { id: "owner-a" }, getAccessToken: vi.fn(async () => "token-a"), login: vi.fn() }));
...
it("persists a submitted question then reloads owned state, disabling duplicate writes", async () => {
  ...
  vi.stubGlobal("fetch", fetcher); render(<LiveFanSubmissions slug="event" locale="ko" />);
...
it("uses server member gate and removes old owner's visible data before the next read", async () => {
  ...
  vi.stubGlobal("fetch", fetcher); const view = render(<LiveFanSubmissions slug="event" locale="ko" />);
  await screen.findByText(/Fan Passport가/); expect(screen.queryByRole("textbox")).not.toBeInTheDocument(); expect(screen.queryByText("질문입니다")).not.toBeInTheDocument();
  ...
  view.rerender(<LiveFanSubmissions slug="event" locale="ko" />);
...
it("closes an already rendered form at the actual deadline", async () => {
 ...
 try {
  ...
  await act(async()=>{render(<LiveFanSubmissions slug="event" locale="ko"/>);await vi.advanceTimersByTimeAsync(1);});
