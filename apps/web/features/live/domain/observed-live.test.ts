...
describe("observed LIVE feed merging", () => {
  it("defaults legacy cards to TikTok while distinguishing the same creator on YouTube", () => {
    ...
    expect(mergeObservedLiveFeed([], feed([tiktok, youtube]), now)).toEqual([tiktok, youtube]);
  });
