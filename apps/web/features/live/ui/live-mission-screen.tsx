...
export function LiveMissionScreen({slug,locale}:{slug:string;locale:"ko"|"en"}){
  ...
  return <main className={styles.page}><header><p>LIVE · {slug}</p><h1>{locale==="ko"?"미션":"Missions"}</h1></header>{message&&<p role="status" className={styles.notice}>{message}</p>}{missions.length===0?<p>{locale==="ko"?"진행할 수 있는 미션이 없습니다.":"No missions are available."}</p>:missions.map(m=><article key={m.id} className={styles.card}><span>{m.type.toUpperCase()}</span><h2>{m.title}</h2><p>{m.description}</p>{m.questions.map(q=><fieldset key={q.id}><legend>{q.text}</legend>{q.media&&<Media value={q.media}/>} {q.options.map(o=><label key={o.id} className={styles.option}><input type="radio" name={q.id} checked={answers[q.id]===o.id} onChange={()=>setAnswers(v=>({...v,[q.id]:o.id}))}/>{o.media&&<Media value={o.media}/>}<span>{o.label}</span></label>)}</fieldset>)}<button disabled={m.completed||m.questions.some(q=>!answers[q.id])} onClick={()=>void submit(m)}>{m.completed?(locale==="ko"?"완료됨":"Completed"):(locale==="ko"?"미션 완료":"Complete mission")}</button></article>)}</main>;
}
function Media({value}:{value:{type:"image"|"video";url:string}}){return value.type==="video"?<video controls preload="metadata" src={value.url}/>:<Image unoptimized src={value.url} alt="" width={640} height={360}/>;}
