...
function OwnerHistory({ locale, kind, auth }: { locale: AppLocale; kind: HistoryKind; auth: ReturnType<typeof usePrivy> }) {
  ...
  return <FanAppFrame locale={locale} currentPath="/my" mainId="activity-history"><FanContentContainer as="main" id="activity-history" className={styles.main} tabIndex={-1}>
    <Link href={`/my?locale=${locale}`}>{t.back}</Link><h1>{t.title}</h1>
    ...
    {!auth.ready ? <FanState kind="loading" title={t.loading}/> : !auth.authenticated ? <FanAction href={`/login?locale=${locale}&returnTo=${encodeURIComponent(returnTo)}` as Route}>{t.login}</FanAction>
      ...
      : <><ul className={styles.rows}>{rows.map(row => <li key={row.id}><Link href={withLocalePath(row.href, locale) as Route}><strong>{row.title}</strong><span>{row.status in rewardStatusCopy ? rewardStatusCopy[row.status as keyof typeof rewardStatusCopy][locale] : t[row.status as "submitted" | "selected" | "cancelled" | "collected"]}</span><time dateTime={row.occurredAt}>{new Intl.DateTimeFormat(locale, { dateStyle: "medium" }).format(new Date(row.occurredAt))}</time></Link></li>)}</ul>{!rows.length ? <p role="status">{t.empty}</p> : null}
