...
describe("MY next benefit", () => {
  it("fetches only the initial representative and can reveal another creator's eligible benefit", async () => {
    ...
    render(<MyBenefitProgress creators={creators} locale="ko"/>);
    expect(await screen.findByText("현재 이 최애의 다음 혜택이 없어요.")).toBeInTheDocument();
    expect(fetcher).toHaveBeenCalledTimes(1);
    expect(screen.getByRole("combobox")).toHaveValue(ids[0]);
    fireEvent.change(screen.getByRole("combobox"), { target: { value: ids[1] } });
  ...
  it("renders score progress separately from every other unmet condition", async () => {
    ...
    render(<MyBenefitProgress creators={[creators[0]]} locale="ko"/>);
  ...
  it.each([
    ...
    render(<MyBenefitProgress creators={[creators[0]]} locale="en"/>);
  ...
  it("drops the previous owner's and language's content while replacement loads", async () => {
    ...
    vi.stubGlobal("fetch", fetcher);
    const view = render(<MyBenefitProgress creators={[creators[0]]} locale="ko"/>);
    await screen.findByRole("heading", { name: benefit.title });
    owner.id = "owner-b";
    view.rerender(<MyBenefitProgress creators={[creators[0]]} locale="en"/>);
