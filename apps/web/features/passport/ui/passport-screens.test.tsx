...
import { PassportCollectionScreen, PassportDetailScreen, StampDetailScreen } from "./passport-screens";

const getAccessToken = vi.fn(async () => "access-token");
const push = vi.fn();
let locale = "ko";
vi.mock("@privy-io/react-auth", () => ({ usePrivy: () => ({ ready: true, authenticated: true, getAccessToken }) }));
vi.mock("next/navigation", () => ({ usePathname: () => "/passports", useRouter: () => ({ push }), useSearchParams: () => new URLSearchParams(`locale=${locale}`) }));
...
describe("passport fan screens", () => {
  beforeEach(() => { locale = "ko"; vi.clearAllMocks(); delete process.env.NEXT_PUBLIC_BLOCKCHAIN_EXPLORER_TX_BASE; });
