...
export function FanTicketGuide({ creatorSlug, creatorName, locale, compact = false }: { creatorSlug: string; creatorName: string; locale: FanLocale; compact?: boolean }) {
  ...
  const visibleActions = compact ? data.actions.slice(0, 4) : data.actions;
  return <section className={`${styles.guide} ${compact ? styles.compact : ""}`} aria-labelledby={`ticket-guide-${creatorSlug}`}>
    ...
    <div className={styles.actions}>{visibleActions.map((action) => <ActionRow key={action.key} action={action} locale={locale} />)}</div>
