...
function deps(): BenefitFulfillmentRouteDependencies { return { authorize: vi.fn(async () => ({ appUserId: "owner" })), repository: { saveRecipient: vi.fn(async () => ({ winnerId, method: "physical_shipping" as const, status: "ready" as const, revision: 2 })) } }; }
