...
import { createSupabasePassportReadRepository } from "./passport-read-repository";
import type { PassportReadRouteDependencies } from "./passport-read-route";

export function createPassportReadRouteDependencies(): PassportReadRouteDependencies {
  ...
  return {
    ...
    repository: createSupabasePassportReadRepository({ url: environment.SUPABASE_URL, serviceRoleKey: environment.SUPABASE_SERVICE_ROLE_KEY }, database),
  };
}

export function passportReadUnavailableResponse(): Response { return Response.json({ error: { code: "PASSPORTS_UNAVAILABLE" } }, { status: 503, headers: { "cache-control": "no-store", vary: "Authorization" } }); }
