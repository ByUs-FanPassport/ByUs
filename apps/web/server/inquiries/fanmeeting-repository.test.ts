...
it("only forwards the fixed submit contract", async () => {
  ...
  expect(await createInquiryRepository({ rpc }).submit(input, "a".repeat(64), "b".repeat(64))).toBe(false);
...
it.each(["INQUIRY_RATE_LIMITED", "INQUIRY_IDEMPOTENCY_CONFLICT", "INQUIRY_INVALID"])("maps exact safe database code %s", async code => {
  ...
  await expect(createInquiryRepository({ rpc }).submit(input,"a","b")).rejects.toMatchObject({ code });
});
it("rejects unexpected result and masks SQL details", async () => {
  ...
  await expect(repo.submit(input,"a","b")).rejects.toMatchObject({ code: "INQUIRY_UNAVAILABLE" });
  rpc.mockResolvedValue({ error: { message: "PII detail" }, data: null });
  await expect(repo.submit(input,"a","b")).rejects.toMatchObject({ message: "INQUIRY_UNAVAILABLE" });
});
