import "server-only";
import { InquiryError, type InquiryInput, type InquiryRepository } from "./fanmeeting-route";
...
export function createInquiryRepository(client: RpcClient): InquiryRepository {
  return { async submit(input: InquiryInput, ipHash: string, payloadHash: string) {
    const { data, error } = await client.rpc("submit_business_inquiry", {
