...
const state=vi.hoisted(()=>({queue:vi.fn(),external:vi.fn(),ses:vi.fn(),http:vi.fn(),inquiry:vi.fn(),reminders:vi.fn(),kakaoQueue:vi.fn(),kakaoWorker:vi.fn(),solapi:vi.fn()}));
...
beforeEach(()=>{vi.clearAllMocks();state.inquiry.mockResolvedValue(0);state.reminders.mockResolvedValue(0);state.kakaoQueue.mockReturnValue({});});
