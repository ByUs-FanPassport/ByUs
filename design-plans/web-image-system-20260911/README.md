...
### 모바일 홈 배너 통일 — 사용자 최종 수정 (2026-09-11)

모바일 홈의 세 슬라이드는 사용자 지정인 ‘세로로 긴 사진 위에 정보 배치’로 통일한다. 4:5 캔버스(최소 높이 400px), 하단 정보 오버레이, 같은 버튼 위치를 공유하며 사진 원본 비율에 따라 슬라이드 높이를 바꾸지 않는다. 모바일 LIVE는 상위 셀럽의 `portrait` 역할을 `CreatorImage`의 `editorial` 슬롯으로 전달한다. PC LIVE는 기존 행사 이미지 전달을 유지하고, 엘리나 참여 가이드는 별도로 요청한 밝은 좌우 분할 배너를 사용한다.

로컬 검증: 한국어·영어, 320/390/1440px에서 세 슬라이드 전환을 확인했다. 모바일 세 장의 높이·상단·버튼 위치가 일치하고 정보가 사진 안에 들어가며, 가로 넘침·정보/버튼/이동 컨트롤 겹침이 없다. 관련 61개 테스트, 타입 검사, ESLint 통과.
