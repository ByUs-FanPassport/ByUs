...
현재 worktree에 구현했다. 운영 데이터 변경, 푸시, 배포는 하지 않았다. 이후 배포할 때에는 `20260912010000_fan_lounge.sql`을 해당 환경에 먼저 적용한 뒤 웹을 반영해야 한다. 기존 `npm run dev`는 운영 DB를 참조할 수 있으므로 쓰기 검증에 사용하지 않는다.
