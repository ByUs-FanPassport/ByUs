...
The private `/api/internal/instagram/live-sync` cron runs once per minute on the verified Vercel Pro team, separately from hourly media collection. Each account is due every three minutes. It claims published creators with a valid connected token and a matching active profile; a ByUs LIVE event is no longer required. Each invocation claims at most 25 accounts, processes five concurrently, and bounds each source request to five seconds. Separate 45-second leases and generation/token checks reject duplicate or late writes. More than 25 eligible accounts can delay detection; stale observations always fall back rather than extending freshness.

The public watch handler reads only whitelisted cached observation fields. It never retrieves or decrypts an Instagram token. Automatic redirects require a matching creator/account, an actual broadcast start within the event window, and an observation younger than five minutes; the window is checked again after the read. Confirmed offline responses, expired cache/token, disconnected accounts and missing connections preserve the stored profile URL. Failed probes preserve an earlier valid observation until its original five-minute expiry. Connection or token changes invalidate previous observations. Detection and end propagation follow the three-minute per-account schedule; this is not instantaneous detection.
...
## Instagram home discovery and owned-account validation, 2026-09-12

The requested scope is home/LIVE-list implementation and test-account verification before asking celebrities to connect. The existing global integration gate stays unchanged; no celebrity was contacted and no production creator connection was created with the test credential.

`/api/public/live-now?v=3` adds Instagram to the existing TikTok/YouTube feed and shared card component. v1 remains TikTok-only, v2 remains TikTok/YouTube so older clients do not mislabel Instagram. Every published Instagram social profile receives the same public unavailable shape when disconnected, disabled, expired, missing or temporarily unreadable. Connection eligibility and private error reasons are not exposed through fields or target omission. A public creator/profile removal prunes its card on the next successful roster read. After a private connection is removed, the database stops serving its observation immediately; a browser can retain an already received card only until its original five-minute expiry.

The source is unchanged and accepts the previously verified same-owner `BROADCAST + FEED` stories permalink. No audience discriminator was added: the user's earlier decision to support the observed official permalink remains in force. Instagram determines viewer access. New cards use the public creator name/image and an Instagram-specific watch link. No private token is read or decrypted in the public request path; only the private sync worker opens an identity-bound token.

Migration `20260912131855_instagram_live_home_discovery.sql` removes the event requirement from the existing claim function, sets the next attempt three minutes ahead, preserves an original fresh LIVE observation on unknown/error, and adds the strict service-role-only discovery projection. Confirmed offline overwrites LIVE immediately. Existing generation/token-change/disconnect erasure, lease checks, exact active social profile matching, row access controls and manual/event watch rules remain. Instagram feed/client/watch expiry is five minutes; TikTok and YouTube timings are unchanged. The matching rollback restores event-scoped collection and prior read behavior without deleting connection records.

### Evidence boundaries

- Fresh owned-account API at `2026-09-12T13:19:10.943Z`: `mirrorworld.ai`, `MEDIA_CREATOR`, HTTP 200, empty `live_media`, offline matched. Evidence: `artifacts/instagram-home-20260912/current-account-proof.json`.
- Actual local pipeline: a disposable PostgreSQL database held a local test connection for that exact owner, with the real credential sealed using a random in-memory vault key. The actual claim/repository/sync worker decrypted it, called Instagram, persisted offline, read the new private projection and returned an offline target/no card from the actual v3 route with zero registered LIVE events. This verifies the new worker/storage/feed path with the owned account; the local connection metadata is a test fixture, not a production OAuth callback. The database was dropped and the key was not persisted. Evidence: `real-local-pipeline.json` and repeatable `verify-pipeline.mjs` beside it.
- Positive/end controls reuse the actual Everyone responses captured at `12:01:40.326Z` and `12:03:57.529Z`. The real source, strict cached reader and v3 route were exercised at those original timestamps. KO/EN feed artifacts preserve those timestamps exactly. This is historical response replay, not a claim that the account is currently broadcasting or that a fan played it in Instagram.
- Isolated PostgreSQL tests pass for no-event eligibility, three-minute scheduling, five-minute expiry, unknown retention without redating, offline removal, generation/token races, disconnection erasure, private reads and service-role writes. TypeScript and scoped lint passed. Focused web tests passed: 61 reader/sync/watch and 36 feed/domain/UI; unchanged source/OAuth evidence was reused.

Local browser replay passed with the actual component and exact KO/EN v3 handler outputs at 375px/1440px. It verified the Instagram CTA, keyboard new-tab destination, 299/300-second expiry, immediate confirmed-end removal, no overflow and no browser errors. Evidence: `artifacts/instagram-home-20260912/ui/browser-proof.json`; external destinations were intercepted. Production migration history was recorded and read-back confirmed no event requirement, three-minute schedule, uniform unavailable fallback and denied anon/authenticated discovery RPC execution. The global integration flag and production creator connections were not modified.
