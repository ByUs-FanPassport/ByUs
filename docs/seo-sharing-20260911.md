...
- 관련 Vitest 7개 파일, 103개 테스트 통과. 메타·사이트맵·프록시·LIVE 화면/저장소·관리자 인증·MY 혜택 API 포함.
- `npm run typecheck`, `npm run lint:web`, `npm run build` 통과. 잠금 파일의 Next.js 16.3.4 사용.
- `node scripts/verify-seo-local.mjs` 통과: 로컬 production build + 공개 운영 데이터 읽기. 8개 경로×ko/en=16개 초기 HTML, 봇 6종, 사이트맵 34개 고유 URL, 개인 경로 6개 noindex, 비로그인 API 3개 401, 없는 LIVE/셀럽 2개 404.
...
- [ ] 변경 커밋을 main에 통합하고 배포의 최종 성공 상태 확인.
...
- [ ] 네이버: 기존 KimBeautySong 계정 사용. 서비스 발급 `naver88e4a5e89f0454d717836b75fd7b58a0.html` 파일을 public에 추가. 배포 후 인증 대기.
- [ ] 각 서비스에 `https://byus.kr/sitemap.xml` 제출. 홈·이퓨 가이드·LIVE·셀럽 URL 검사와 수집 요청 결과 기록.
- [ ] 카카오·Meta 공유 디버거에서 위 4개 URL 재수집. 실제 카카오톡·X·Telegram 미리보기의 제목·이미지·한영 표시 확인. 실제 메시지 게시·전송은 수신처와 명시적 전송 승인 필요.
- [ ] 운영 방화벽과 봇별 접근 상태 확인. 로컬 UA 응답 검증만으로 운영 방화벽 통과를 단정하지 않음.
...
- [x] 모바일 성능 기준선: 동일 Lighthouse 13.4.1, 390×844 DPR2, simulated RTT150ms/1638.4Kbps/CPU4 조건으로 홈·이퓨 가이드·LIVE 각3회. 실제 이용자 Core Web Vitals가 아닌 로컬 실험실 수치다. LIVE CLS 1.287을 확인해 수정·재검증 중.
- [ ] 유입→팬 인증→예약→출석→응모 분석: 기존 완료 이벤트에 개인정보를 제한한 유입 이벤트 추가, 읽기 전용 집계 SQL 준비. 상세 `docs/seo-conversion-20260911.md`.
- [ ] 색인 제외 및 AI 검색 실적의 계정 화면 현재 상태 확인.
