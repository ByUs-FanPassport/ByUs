...
## 2026-09-13 활동별 로그인 진입

기존 두 익명 로그인 이벤트에 선택 `entryAction`을 추가한다. 실제 출석·응원·Passport 공유 복귀 경로만 고정 enum으로 분류하고 URL/token을 보존하지 않는다. 집계는 `loginEntryActions`이며 legacy `unknown`과 측정된 `other`, 최근30분을 분리한다. 초대는 인증 이후 행동으로 원장 집계를 유지한다. 정확한 계약과 검증 경계는 [활동별 로그인 계측](login-entry-measurement-20260913.md)을 따른다.
