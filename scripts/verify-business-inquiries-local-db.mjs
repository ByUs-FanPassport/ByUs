...
const submit=(id=randomUUID(),ip='a',hash='b',inquiryType)=>inquiryType
 ? `select public.submit_categorized_business_inquiry('${id}','ko','Contact','Company','sender@example.com','Inquiry',true,repeat('${ip}',64),repeat('${hash}',64),'${inquiryType}');`
 : `select public.submit_business_inquiry('${id}','ko','Contact','Company','sender@example.com','Inquiry',true,repeat('${ip}',64),repeat('${hash}',64));`;
...
try{
 ...
 const legacyUpgradeId=randomUUID();sql(asService(submit(legacyUpgradeId)));
 sql(await readFile(new URL('../supabase/migrations/20260911081906_categorize_business_inquiries.sql',import.meta.url),'utf8'));
 check(sql(`select inquiry_type||':'||payload_hash from public.business_inquiries where id='${legacyUpgradeId}'`)==`fanmeeting:${'b'.repeat(64)}`,'legacy upgrade default and hash');
 ...
 for(const role of ['anon','authenticated']){
  ...
  for(const call of [submit(),submit(randomUUID(),'a','b','creator'),'select public.maintain_business_inquiries();','select * from public.claim_business_inquiry();','select public.business_inquiry_health();',`select public.begin_business_inquiry_send('${randomUUID()}','${randomUUID()}');`,`select public.finish_business_inquiry('${randomUUID()}','${randomUUID()}','unknown');`])check(sql(`set role ${role};${call}`,url,true).includes('permission denied'),'RPC access');
 }
 check(sql(asService(submit().replace("'Contact'","''")),url,true).includes('INQUIRY_INVALID'),'invalid');
 check(sql(asService(submit(randomUUID(),'a','b','fanmeeting')),url,true).includes('INQUIRY_INVALID'),'typed RPC rejects legacy/arbitrary category');
 sql('truncate public.business_inquiries');
 const categorizedId=randomUUID();check(sql(asService(submit(categorizedId,'a','b','creator')))==='f','categorized new');
 check(sql(`select inquiry_type from public.business_inquiries where id='${categorizedId}'`)==='creator','categorized stored type');
 check(sql(asService(submit(categorizedId,'a','b','creator')))==='t','categorized replay');
 check(sql(asService(submit(categorizedId,'a','b','partner')),url,true).includes('INQUIRY_IDEMPOTENCY_CONFLICT'),'category conflict');
 sql('truncate public.business_inquiries');
 ...
 const rate=await Promise.all(Array.from({length:8},(_,i)=>parallel(asService(submit(randomUUID(),'a','b',i%2===0?'creator':'partner')))));
 ...
 const global=await Promise.all(['a','d','e','f'].map((ip,i)=>parallel(asService(submit(randomUUID(),ip,'b',i%2===0?'creator':'partner')))));
 ...
 console.log('PASS: legacy upgrade/hash, category validation, permissions, atomic idempotency/shared IP/global quotas, exclusive claim, durable sending recovery, attempt ownership, bounded retries, PII erasure, retention. Cron scheduling contract only (fixture stub).');
