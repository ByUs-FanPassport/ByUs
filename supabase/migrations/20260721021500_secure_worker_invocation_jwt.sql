-- Require both Supabase platform JWT verification and the dedicated cron
-- shared secret for worker invocation requests.

drop function if exists public.configure_byus_worker_invocation(text, text);
...
begin
  ...
  if p_supabase_anon_key is null or length(p_supabase_anon_key) < 40 then
    raise exception 'a valid Supabase anon key is required';
  end if;
  ...
  select id into existing_anon_id
  from vault.secrets
  where name = 'byus_worker_anon_key'
  order by created_at desc
  limit 1;
  ...
  if existing_anon_id is null then
    perform vault.create_secret(
      p_supabase_anon_key,
      'byus_worker_anon_key',
      'Supabase anon JWT used for platform verification of worker cron requests'
    );
  else
    perform vault.update_secret(
      existing_anon_id,
  ...
  for existing_job_id in
    select jobid from cron.job where jobname = 'byus-invoke-mint-worker-http'
  loop
...
revoke all on function public.configure_byus_worker_invocation(text, text, text)
from public, anon, authenticated;
grant execute on function public.configure_byus_worker_invocation(text, text, text)
to service_role;
...
as $$
  select jsonb_build_object(
    ...
    'pg_net_extension', exists (select 1 from pg_extension where extname = 'pg_net'),
    'vault_extension', exists (select 1 from pg_extension where extname = 'supabase_vault'),
    'cms_assets_bucket', exists (select 1 from storage.buckets where id = 'cms-assets'),
    'queue_maintenance_cron', exists (
      select 1 from cron.job where jobname = 'byus-queue-maintenance-http' and active
    ),
    'cron_vault_secret', exists (select 1 from vault.secrets where name = 'byus_cron_secret'),
    ...
    'worker_anon_key_vault_secret', exists (
      select 1 from vault.secrets where name = 'byus_worker_anon_key'
    )
  );
$$;
