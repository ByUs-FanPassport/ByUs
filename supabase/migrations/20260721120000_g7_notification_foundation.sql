...
begin
  ...
  if found and v_existing.app_user_id<>p_app_user_id then
    ...
    update public.push_subscriptions subscription set app_user_id=p_app_user_id,endpoint=p_endpoint,p256dh=p_p256dh,auth_secret=p_auth_secret,user_agent=p_user_agent,disabled_at=null where subscription.id=v_existing.id returning subscription.id into v_subscription_id;
  elsif found then
    update public.push_subscriptions subscription set endpoint=p_endpoint,p256dh=p_p256dh,auth_secret=p_auth_secret,user_agent=p_user_agent,disabled_at=null where subscription.id=v_existing.id returning subscription.id into v_subscription_id;
  else
    insert into public.push_subscriptions(app_user_id,endpoint,endpoint_hash,p256dh,auth_secret,user_agent) values(p_app_user_id,p_endpoint,p_endpoint_hash,p_p256dh,p_auth_secret,p_user_agent) returning push_subscriptions.id into v_subscription_id;
  end if;
