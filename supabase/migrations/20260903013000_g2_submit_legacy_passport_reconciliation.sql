-- Reconcile an exact completed legacy Passport job instead of minting a duplicate.
-- This replaces the submit function forward-only after the legacy orphan was observed on Dev.
...
begin
  ...
  insert into public.quiz_passes(
    ...
    v_pass_id, p_app_user_id, attempt_record.celebrity_id, attempt_record.id
  )
  ...
  if found then
    if job_record.entity_type <> 'passport'
       ...
       or job_record.payload - 'workerSubmission' <> expected_payload
