-- Required ordered roles: first is representative, all participate in discovery.
-- Source review and all 15 existing assignments: docs/creator-role-classification-20260910.md.
-- Apply this complete transaction before the web release; unknown rows abort backfill.
begin;

create type public.celebrity_role as enum ('artist', 'creator', 'show_host');
alter table public.celebrities add column roles public.celebrity_role[];

-- Archived profiles also need the new field. Suspend only their update guard
-- while filling this column; the transaction's table lock excludes other writers.
alter table public.celebrities disable trigger celebrities_enforce_lifecycle;

update public.celebrities c set roles = source.roles::public.celebrity_role[]
from (values
  ('aryeom', array['artist','creator']),
  ('changha', array['creator','artist']),
  ('ddg', array['artist','creator']),
  ('elina', array['creator','artist']),
  ('ifewknow', array['artist','creator']),
  ('jenny-jeong', array['creator']),
  ('kara', array['artist']),
  ('katseye', array['artist']),
  ('megan-lenius', array['artist','creator']),
  ('nick-black', array['artist','creator']),
  ('park-myungho', array['artist']),
  ('t-pain', array['artist','creator']),
  ('thisisj-official', array['show_host','creator']),
  ('xin', array['artist']),
  ('yuna', array['creator','artist'])
) source(slug, roles)
where source.slug = c.slug;

do $$
declare missing_slugs text;
begin
  select string_agg(slug, ', ' order by slug) into missing_slugs
  from public.celebrities where roles is null or cardinality(roles) = 0;
  if missing_slugs is not null then
    raise exception 'celebrity roles backfill required: %', missing_slugs;
  end if;
end $$;

-- Flush the existing deferred publication checks before altering the same table.
set constraints all immediate;
alter table public.celebrities enable trigger celebrities_enforce_lifecycle;

alter table public.celebrities
  alter column roles set not null,
  add constraint celebrities_roles_valid check (
    coalesce(array_ndims(roles) = 1 and array_lower(roles, 1) = 1, false)
    and cardinality(roles) between 1 and 3
    and array_position(roles, null) is null
    and (cardinality(roles) < 2 or roles[1] <> roles[2])
    and (cardinality(roles) < 3 or (roles[1] <> roles[3] and roles[2] <> roles[3]))
  );
comment on column public.celebrities.roles is
  'Required ordered role codes. First role is representative; no default or unclassified value.';
...
begin
  ...
  if p_payload ? 'roles' then
    ...
    raise exception 'celebrity roles are required';
  end if;
  ...
  if p_celebrity is null then
    insert into public.celebrities(
      id, slug, image_url, image_position, display_order, fan_count, roles
    ) values (
      target,
      p_payload->>'slug',
      p_payload->>'imageUrl',
