-- Single representative role. Legacy roles remain readable during DB-first rollout.
-- Production must use the locked exact-nine roster guard in the release apply wrapper.
begin;

create type public.celebrity_primary_role as enum ('idol','singer','actor','creator','show_host');
alter table public.celebrities add column primary_role public.celebrity_primary_role;

-- Only published rows are recategorized. Existing draft/archived data is untouched.
update public.celebrities c set primary_role = source.role::public.celebrity_primary_role
from (values
  ('xin','idol'),
  ('park-myungho','singer'),
  ('aryeom','actor'),
  ...
  ('jenny-jeong','creator'),
  ('ifewknow','creator'),
  ('thisisj-official','show_host'),
  -- Historical repository seed replay only; both are non-public in the live roster.
  ('katseye','idol'),
  ('kara','idol')
) source(slug,role)
where c.slug = source.slug and c.status = 'published';

do $$
declare missing_slugs text;
begin
  select string_agg(slug, ', ' order by slug) into missing_slugs
  from public.celebrities where status = 'published' and primary_role is null;
  if missing_slugs is not null then
    raise exception 'published celebrity primary role mapping required: %', missing_slugs;
  end if;
end $$;

set constraints all immediate;
alter table public.celebrities add constraint celebrities_published_primary_role_required
  check (status <> 'published' or primary_role is not null);
comment on column public.celebrities.primary_role is
  'One required public role. Existing non-public profiles must select a role before publication.';
comment on column public.celebrities.roles is
  'Legacy activity codes retained for old client compatibility; public UI uses primary_role.';
...
declare
  ...
  requested_roles public.celebrity_role[];
  requested_primary_role public.celebrity_primary_role;
begin
  ...
  if p_payload ? 'primaryRole' then
    if jsonb_typeof(p_payload->'primaryRole') is distinct from 'string'
      or (p_payload->>'primaryRole') not in ('idol','singer','actor','creator','show_host') then
      raise exception 'invalid celebrity primary role';
    end if;
    requested_primary_role := (p_payload->>'primaryRole')::public.celebrity_primary_role;
  elsif p_celebrity is not null then
    -- Existing edits from old clients keep the primary role under the same row lock.
    select primary_role into requested_primary_role from public.celebrities where id = target;
  else
    raise exception 'celebrity creation requires a primary role';
  end if;

  if p_payload ? 'roles' then
    if jsonb_typeof(p_payload->'roles') <> 'array' then
      raise exception 'invalid celebrity roles';
    end if;
    if exists (select 1 from jsonb_array_elements(p_payload->'roles') r(value)
      where jsonb_typeof(r.value) <> 'string') then
      raise exception 'invalid celebrity roles';
    end if;
    select array_agg(value::public.celebrity_role order by position)
      into requested_roles
    from jsonb_array_elements_text(p_payload->'roles') with ordinality r(value, position);
    if requested_roles is null then raise exception 'celebrity roles are required'; end if;
  elsif p_celebrity is not null then
    -- The row was locked above. Omission from an old client preserves its roles.
    select roles into requested_roles from public.celebrities where id = target;
  else
    -- Compatibility data for the previous web release only. New UI reads primary_role.
    requested_roles := array[case
      when requested_primary_role = 'creator' then 'creator'::public.celebrity_role
      when requested_primary_role = 'show_host' then 'show_host'::public.celebrity_role
      else 'artist'::public.celebrity_role
    end];
  end if;
  ...
  if p_celebrity is null then
    insert into public.celebrities(
      id, slug, image_url, image_position, display_order, fan_count, roles, primary_role
    ) values (
      target,
      p_payload->>'slug',
      p_payload->>'imageUrl',
    ...
    set slug = p_payload->>'slug',
        ...
        primary_role = requested_primary_role
    where id = target;
  end if;
...
begin
  ...
  if p_publish then
    if (select primary_role from public.celebrities where id = p_celebrity) is null then
      raise exception 'celebrity publication requires a primary role';
    end if;
