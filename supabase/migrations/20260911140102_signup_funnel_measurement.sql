-- Signup funnel measurement extends the append-only event ledger without
-- changing the meaning or validation of the original sixteen event names.

alter table public.fan_product_events
  drop constraint fan_product_events_event_name_check,
  add constraint fan_product_events_event_name_check check (event_name in (
    ...
    if p_event_name in ('signup_guide_view','signup_guide_cta') then
      ...
      if p_source <> 'signup.login'
         ...
         or (p_event_name = 'login_started' and (
           (select count(*) from pg_catalog.jsonb_object_keys(p_properties)) <> 8
         ...
         or (p_event_name = 'login_result' and (
           (select count(*) from pg_catalog.jsonb_object_keys(p_properties)) <> 11
...
create function public.project_signup_commit_event_v1()
...
begin
  ...
  begin
    perform public.record_product_event_v1(
      1::smallint,event_name,owner_id,null,null,null,null,null,
      'server.commit_projection','commit:' || source_row || ':' || event_name,
      event_time,'{}'::jsonb
    );
  exception when others then
    raise warning 'signup commit event projection skipped: % %', event_name, sqlstate;
  end;
  return new;
end;
$$;

revoke all on function public.project_signup_commit_event_v1()
  from public,anon,authenticated,service_role;

create trigger app_users_signup_commit_event
after insert on public.app_users
for each row execute function public.project_signup_commit_event_v1();

create trigger user_profiles_signup_commit_event
