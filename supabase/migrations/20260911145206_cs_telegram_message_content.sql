-- User-approved CS message content in the existing operator room.
-- Resolve at claim time; do not copy bodies into retained outbox rows.
-- Preserve the previous RPC shape for rolling deployment and rollback.
create function public.claim_telegram_alert_batch_with_cs_content(p_chat_id text) returns jsonb
...
begin
  ...
  with candidates as (
    ...
    update public.telegram_alert_outbox o set status='claimed',batch_id=token,lease_expires_at=t+interval '60 seconds'
      ...
      then jsonb_build_object('inquiry_id',m.inquiry_id,'message_body',m.body) else '{}'::jsonb end order by c.created_at,c.id)) into result
    from claimed c
