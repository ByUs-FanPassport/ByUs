-- LIVE polling is independent of hourly profile/media collection. Browser grants stay revoked.
alter table public.instagram_connections
  add column live_observation jsonb,
  add column live_generation uuid,
  add column live_next_sync_at timestamptz not null default now(),
  add column live_lease_id uuid,
  add column live_lease_until timestamptz;

create function public.instagram_clear_live_observation()
returns trigger language plpgsql security invoker set search_path = '' as $$
begin
  if new.generation is distinct from old.generation or new.identity is distinct from old.identity
    or new.token_ciphertext is distinct from old.token_ciphertext then
    new.live_observation := null; new.live_generation := null;
    new.live_lease_id := null; new.live_lease_until := null; new.live_next_sync_at := now();
  end if;
  return new;
end;
$$;
create trigger instagram_clear_live_on_connection_change
before update on public.instagram_connections for each row execute function public.instagram_clear_live_observation();

create function public.instagram_claim_live_sync(p_limit integer default 25)
...
create function public.instagram_read_live_observation(p_creator_slug text,p_username text)
