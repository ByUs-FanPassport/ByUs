-- YouTube video reconfirmation every three minutes; budgets and access unchanged.
create or replace function public.youtube_claim_live_api(p_cache_key text, p_stage text)
...
begin
  ...
  ttl := case p_stage when 'channels' then interval '24 hours' when 'search' then interval '1 hour' else interval '3 minutes' end;
...
create or replace function public.youtube_finish_live_api(p_cache_key text, p_lease_id uuid, p_payload jsonb)
...
begin
  ...
  update public.youtube_live_api_cache set
    ...
    next_retry_at = case when p_payload is not null then '-infinity'::timestamptz else moment + case cached.stage when 'search' then interval '1 hour' when 'videos' then interval '3 minutes' else interval '60 seconds' end end,
