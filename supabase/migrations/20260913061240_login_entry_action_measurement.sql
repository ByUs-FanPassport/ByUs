-- Add an optional fixed login entry category. Preserve existing observations,
-- anonymity, wallet diagnostics, replay semantics, RPC signature and privileges.
...
begin
  ...
  if p_properties ? 'entryAction' and (
    p_event_name not in ('login_started','login_result')
    or jsonb_typeof(p_properties->'entryAction') <> 'string'
    or p_properties->>'entryAction' not in ('daily_checkin','cheer','passport_share','other')
  ) then
    raise exception 'PRODUCT_EVENT_INVALID' using errcode = '22023';
  end if;
  ...
  if is_signup_client_event then
    ...
    if p_event_name in ('signup_guide_view','signup_guide_cta') then
      ...
      property_count := (select count(*) from pg_catalog.jsonb_object_keys(p_properties))
        - case when p_properties ? 'entryAction' then 1 else 0 end;
