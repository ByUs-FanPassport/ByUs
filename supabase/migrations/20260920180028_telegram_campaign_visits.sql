-- Deploy the worker accepting campaign_visited before applying this migration.
-- Existing room activation, throttle, leases, delivery outcomes and retention apply.
...
alter table public.telegram_alert_outbox add constraint telegram_alert_outbox_kind_check
  check(kind in ('member_joined','fan_joined','live_reserved','live_attended','draw_published','cs_inquiry_created','cs_user_replied','campaign_visited'));

create function public.capture_telegram_campaign_visit() returns trigger
language plpgsql security definer set search_path='' as $$
declare cfg public.telegram_alert_settings;
begin
  if new.first_link_id is null then return new; end if;
  ...
  perform 1 from public.campaign_tracking_links where id=new.first_link_id and active;
  if not found then return new; end if;
  insert into public.telegram_alert_outbox(kind,source_id,activation_id,chat_id,occurred_at)
    values('campaign_visited',new.id,cfg.activation_id,cfg.chat_id,new.created_at)
    on conflict(kind,source_id) do nothing;
  return new;
exception when others then
  raise log 'TELEGRAM_CAMPAIGN_CAPTURE_FAILED sqlstate=%',sqlstate;
  return new;
end $$;
revoke all on function public.capture_telegram_campaign_visit() from public,anon,authenticated;
create trigger campaign_visits_telegram_alert after insert on public.campaign_visits
  for each row execute function public.capture_telegram_campaign_visit();
