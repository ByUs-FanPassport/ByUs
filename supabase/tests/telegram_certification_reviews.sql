...
do $$
declare claim jsonb; delivery uuid; token text; first_upload uuid; second_upload uuid; approval jsonb;
begin
  ...
  delivery:=(claim->>'delivery_id')::uuid;token:=claim->>'callback_token';
  ...
  if claim->>'submission_id'<>(select id::text from telegram_certification_test_ids where name='telegram_submission')
    ...
    or jsonb_array_length(claim->'uploads')<>2 or length(token)<>32
  ...
  perform public.record_telegram_certification_delivery(delivery,'-1001234567890','sending');
  begin
    perform public.record_telegram_certification_delivery(delivery,'-1001234567890','sent',second_upload,2,9002);
  ...
  if public.record_telegram_certification_delivery(delivery,'-1001234567890','sent',first_upload,1,9001)->>'status'<>'partial' then
    raise exception 'TELEGRAM_CERTIFICATION_FIRST_UPLOAD_NOT_PARTIAL';
  end if;
  if public.record_telegram_certification_delivery(delivery,'-1001234567890','sent',second_upload,2,9002)->>'status'<>'sent' then
...
do $$
declare saved jsonb; mission uuid; submitted jsonb; claim jsonb; delivery uuid; upload uuid; token text; approval jsonb; submission uuid;
begin
  saved:=public.save_admin_certification_mission_v2(
  ...
  claim:=public.claim_telegram_certification_delivery('-1001234567890');delivery:=(claim->>'delivery_id')::uuid;token:=claim->>'callback_token';upload:=(claim->'uploads'->0->>'upload_id')::uuid;
  perform public.record_telegram_certification_delivery(delivery,'-1001234567890','sending');
  perform public.record_telegram_certification_delivery(delivery,'-1001234567890','sent',upload,1,9011);
...
begin
  ...
  submission:=(submitted->>'id')::uuid;
  claim:=public.claim_telegram_certification_delivery('-1001234567890');delivery:=(claim->>'delivery_id')::uuid;token:=claim->>'callback_token';upload:=(claim->'uploads'->0->>'upload_id')::uuid;
  perform public.record_telegram_certification_delivery(delivery,'-1001234567890','sent',upload,1,9021);
...
do $$
declare saved jsonb; mission uuid; submitted jsonb; claim jsonb; delivery uuid; upload uuid; token text; callback jsonb; submission uuid;
begin
  saved:=public.save_admin_certification_mission_v2(
  ...
  submission:=(submitted->>'id')::uuid;
  claim:=public.claim_telegram_certification_delivery('-1001234567890');delivery:=(claim->>'delivery_id')::uuid;token:=claim->>'callback_token';upload:=(claim->'uploads'->0->>'upload_id')::uuid;
  perform public.record_telegram_certification_delivery(delivery,'-1001234567890','sent',upload,1,9031);
  ...
  if callback->>'outcome'<>'already_processed'
    ...
    or not exists(select 1 from public.certification_submissions where id=submission and review_source='admin_web') then
